...
class ExerciseController extends GetxController {
  ...
  // List<String> exerciseRoutineList = <String>[
  //   "I can exercise more than 10 minutes every day",
  //   "I can not exercise more than 10 minutes every day",
  //   "I do not know",
  // ];
  ...
  Future<void> exerciseQusApi() async {
    isLoading.value = true;
    try {
      var token = await StorageServivce.getToken();
      log("beforeResponse");
      var response = await apiService
          .get("${ApiUrls.getQuestionWithAnswer}?order=16", authToken: token);
      log("afterrResponse");
  ...
  Future<void> exerciseAnswerApi(int order, int id, String answer) async {
    Map<String, dynamic> bodyData = {
      "answer": answer,
      "order": "$order",
      "qId": "$id",
      "PageName": QuestionPageNames.exercisePageName,
    };
    try {
      isLoading.value = true;
      var token = await StorageServivce.getToken();
  ...
  Future<void> favExerciseQusApi() async {
    try {
      var token = await StorageServivce.getToken();
      log("beforeResponse");
      var response = await apiService
          .get("${ApiUrls.getQuestionWithAnswer}?order=17", authToken: token);
      log("afterrResponse");
      if (response.statusCode == 200) {
        var dateObj = jsonDecode(response.body);
