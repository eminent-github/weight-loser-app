...
class ComplianceController extends GetxController {
  ...
  //   "I manage cravings while sticking to the plan",
  //   "I have moderate cravings and occasionally deviate from the plan",
  //   "I struggle with cravings that leads to plan failure",
  // ];
  ...
  Future<void> complianceAnswerApi(int order, int id, String answer) async {
    Map<String, dynamic> bodyData = {
      "answer": answer,
      "order": "$order",
      "qId": "$id",
      "PageName": QuestionPageNames.compliancePageName,
    };
    try {
  ...
  Future<void> stressEatingQusApi() async {
    try {
      var token = await StorageServivce.getToken();
      log("beforeResponse");
      var response = await apiService
          .get("${ApiUrls.getQuestionWithAnswer}?order=24", authToken: token);
      log("afterrResponse");
      if (response.statusCode == 200) {
        var dateObj = jsonDecode(response.body);
